...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --scope      Only sync specific scope (root, site, lore-linter, wiki-cli)"
            exit 0
            ;;
        *)
...
get_agents_path() {
    ...
    case "$scope" in
        ...
        site)       echo "$REPO_ROOT/src/AGENTS.md" ;;
        lore-linter) echo "$REPO_ROOT/packages/lore-linter/AGENTS.md" ;;
        wiki-cli)   echo "$REPO_ROOT/packages/wiki-cli/AGENTS.md" ;;
        *)          echo "" ;;
    esac
}
